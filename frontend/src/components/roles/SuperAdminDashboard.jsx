      ...
      {activeTab === 'users' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-8 card-onyx p-6">
              <div className="flex items-center justify-between pb-3 mb-3 border-b border-zinc-800">
                <div className="flex items-center space-x-2.5">
                  <UserCheck className="w-5 h-5 text-amber-400" />
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-sm font-semibold text-zinc-50">Pending Staff Access Approvals</h2>
              ...
              {pendingUsers.length === 0 ? (
                <div className="py-8 text-center text-zinc-400 text-xs bg-zinc-950 rounded-xl border border-zinc-800 flex items-center justify-center space-x-2">
                ...
                <div className="divide-y divide-zinc-800">
                  {pendingUsers.map((user) => (
                    <div key={user.id} className="py-3 flex items-center justify-between gap-3 hover:bg-zinc-800/40 px-2 rounded-lg transition-colors">
                      <div className="min-w-0">
                        <div className="flex items-center space-x-2">
                          <span className="text-xs font-semibold text-zinc-100 truncate">{user.full_name || user.email}</span>
                          <span className="badge-amber font-mono text-[10px] uppercase">
                            {user.department_domain} Admin
                          </span>
                        </div>
                        <p className="text-[11px] text-zinc-400 font-mono mt-0.5">{user.email}</p>
                      </div>
                      ...
                      <div className="flex items-center space-x-2 shrink-0">
                        ...
                        <button
                          ...
                          disabled={actionLoading === user.id}
                          className="btn-onyx-secondary text-xs py-1 px-2.5 text-rose-400"
                        >
            ...
            <div className="lg:col-span-4 card-onyx p-6 flex flex-col justify-between space-y-4">
              <div>
              ...
              <button
                ...
                className="btn-amber-primary w-full text-xs font-mono"
              >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-8 card-onyx p-6">
              ...
              {pendingUsers.length === 0 ? (
                <div className="py-6 text-center text-zinc-400 text-xs bg-zinc-950 rounded-xl border border-zinc-800 flex items-center justify-center space-x-2">
                  <ShieldCheck className="w-4 h-4 text-emerald-400" />
                  <span>All staff registration requests have been reviewed and verified.</span>
                </div>
              ) : (
                <div className="divide-y divide-zinc-800">
                  {pendingUsers.map((user) => (
                    <div key={user.id} className="py-2.5 flex items-center justify-between gap-3 hover:bg-zinc-800/40 px-2 rounded-lg transition-colors">
                      <div className="min-w-0">
                        <div className="flex items-center space-x-2">
                          <span className="text-xs font-semibold text-zinc-100 truncate">{user.full_name || user.email}</span>
                          <span className="badge-amber font-mono text-[10px] uppercase">
            ...
            <div className="lg:col-span-4 card-onyx p-6 flex flex-col justify-between space-y-4">
              <div>
                ...
                <h3 className="text-sm font-semibold text-zinc-50 flex items-center gap-2">
                  Universal AI Data Intaker
                  <span className="badge-amber text-[9px] font-mono">
                    AI Advisor
                  </span>
                </h3>
                <p className="text-xs text-zinc-400 mt-1 leading-relaxed">

...
export default function SubAdminDashboard({ currentUser }) {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {/* ================= PERFECTLY MATCHED FACULTY DASHBOARD COLOR BANNER BOX ================= */}
      <div className="w-full glass-panel p-6.5 rounded-3xl border dark:border-slate-800/90 border-slate-200 dark:bg-gradient-to-r dark:from-slate-900/95 dark:via-slate-900/90 dark:to-slate-950/95 bg-white shadow-2xl relative overflow-hidden group hover-classic-lift">
        {/* High-Resolution Auditorium Watermark Background Overlay */}
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-15 dark:opacity-20 mix-blend-multiply dark:mix-blend-overlay pointer-events-none rounded-3xl transition-opacity duration-500"
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-5 z-10">
          <div className="space-y-2 max-w-3xl">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              <span className="px-3.5 py-1 text-[10px] sm:text-[11px] font-black uppercase tracking-wider bg-gradient-to-r from-blue-500/20 to-indigo-500/20 text-blue-600 dark:text-blue-400 border border-blue-500/30 rounded-full flex items-center gap-1.5 shadow-sm">
                <Building2 className="w-3.5 h-3.5 text-blue-500" />
                <span>Audi Manager Interface // Class Manager</span>
              </span>
              <span className="text-[10px] sm:text-xs font-mono font-bold px-3 py-0.5 dark:bg-slate-950 bg-slate-100 text-slate-700 dark:text-slate-300 rounded-full border border-slate-200 dark:border-slate-800">
                Domain: <strong className="text-indigo-600 dark:text-indigo-400">EVENTS</strong>
              </span>
            </div>

            {/* Title & Role Line */}
            <div className="flex items-center gap-2.5 flex-wrap pt-0.5">
              <h1 className="text-2xl md:text-3xl font-black tracking-tight dark:text-white text-slate-900 flex items-center gap-2">
                <span>Good morning,</span>
                <span className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 bg-clip-text text-transparent">
                  {currentUser?.full_name || 'Marcus Brody'}
                </span>
              </h1>
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/30 flex items-center gap-1.5">
            ...
            {/* Subtitle Line */}
            <p className="text-xs md:text-sm dark:text-slate-400 text-slate-600 leading-relaxed font-medium">
              Real-time facility occupancy, automated AC climate approvals & hardware telemetry dashboard.
            </p>
          </div>

          {/* Action Button matching Faculty Dashboard primary CTA */}
          <div className="flex items-center gap-3 shrink-0 self-start md:self-center">
            <button
              onClick={fetchSubAdminData}
              disabled={isLoading}
              className="px-5.5 py-3 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-extrabold rounded-2xl shadow-xl shadow-blue-600/25 transition-all transform hover:-translate-y-0.5 cursor-pointer flex items-center gap-2 text-xs md:text-sm"
            >
              <RefreshCw className={`w-4 h-4 text-blue-200 ${isLoading ? 'animate-spin' : ''}`} />
